...
public class TermDocumentIndexer {
	*//**
	...
	public static void main(String[] args) {
		...
		// scanner for getting user's query term
		Scanner in = new Scanner(System.in);

	    System.out.println("Enter the single-term to search for: ");
	    
	    // stores user's term as a string
	    String query = in.nextLine();
		
	    // while loop to continue asking users for terms
		while (!query.equals("quit")) {
			
			// for loop to print documentIDs that match query
			for (Posting p : index.getPostings(query)) {
				System.out.println("Document ID " + p.getDocumentId());
			}
			
			// ask user again for term
			System.out.println("Enter the signle-term to search for: ");
			query = in.nextLine();
		}
