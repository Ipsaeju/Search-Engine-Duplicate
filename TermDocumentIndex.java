...
public class TermDocumentIndex implements Index {
	...
	public List<Posting> getPostings(String term) {
		...
		// if term is in the vocabulary list
		if (vIndex >= 0) {
			
			// walk down the mMatrix row for the term
			for (int column = 0; column < mMatrix[vIndex].length; column++) {
				
				// collect the document IDs (column indices) of the "true" entries.
				if (mMatrix[vIndex][column] == true) {
					
					// create a posting and initialize it's documentId
					Posting p = new Posting(column);
